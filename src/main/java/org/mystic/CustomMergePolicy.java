...
public abstract class CustomMergePolicy extends MergePolicy {
    ...
    private MergeSpecification findForcedMergesSizeLimit(
            SegmentInfos infos, int maxNumSegments, int last, MergeContext mergeContext) throws IOException {
    ...
    public MergeSpecification findForcedMerges(SegmentInfos infos,
        ...
        if (anyTooLarge) {
            return findForcedMergesSizeLimit(infos, maxNumSegments, last, mergeContext);
        } else {
    ...
    public MergeSpecification findForcedDeletesMerges(SegmentInfos segmentInfos, MergeContext mergeContext)
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            int delCount = mergeContext.numDeletedDocs(info);
            if (delCount > 0) {
    ...
    public MergeSpecification findMerges(MergeTrigger mergeTrigger, SegmentInfos infos, MergeContext mergeContext) throws IOException {
        ...
        final Collection<SegmentCommitInfo> mergingSegments = mergeContext.getMergingSegments();
